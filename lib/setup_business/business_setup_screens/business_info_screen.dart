...
class _BusinessInfoScreenState extends State<BusinessInfoScreen> {
  ...
  final years = [for (int i = 2000; i <= DateTime.now().year; i++) i];
  ...
  void _onNext() {
    ...
    if (reg == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Missing owner data")));
      return;
    }
    ...
    provider.setRegistration(
      reg.copyWith(
        ...
        businessStartYear: selectedYear!, // int!
        businessLocation: locationController.text,
        gpsAddress: gpsController.text,
